...
xlabel('Time [s]');
ylabel('North position [m]');
legend('Actual trajectory','Reference_trajectory');
grid on;
...
f8 = figure('Name','Reference_Surge_Velocity_and_actual_velocity_(Body_frame)');
hold on;
...
f9 = figure('Name','Reference_Sway_Velocity_and_actual_velocity_(Body_frame)');
hold on;
...
f10 = figure('Name','Reference_Heading_Velocity_and_actual_velocity_(Body_frame)');
hold on;
...
%       % Output the figure
%       filename = sprintf('Simulation3_%s_%i_%i_%i%i.eps', fig.Name,c(3),c(2),c(4),c(5));
